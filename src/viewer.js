...
export class Viewer {
    ...
    createSpinner() {
        // 1. 注入 CSS 样式
        // 检查是否已经存在该样式，避免重复注入
        if (!document.getElementById('viewer-loader-style')) {
            const style = document.createElement('style');
            style.id = 'viewer-loader-style';
            style.innerHTML = `
        ...
        this.spinnerEl.innerHTML = '<div class="viewer-spinner"></div>';
        this.el.appendChild(this.spinnerEl);
    }

    // --- 新增：显示 Loading ---
    showLoading() {
        if (this.spinnerEl) this.spinnerEl.classList.add('visible');
    }
     ...
     */
    updateTitle(url) {
        // 1. 从 URL 中提取文件名 (例如 "assets/tianma.glb" -> "tianma.glb")
        // 注意：如果是 blob: 开头的 URL（拖拽上传），这里通常匹配不到 info.json，这是符合预期的
        ...
        fetch('models/info.json')
            ...
            .then((data) => {
                ...
                if (data && data[filename]) {
                    const modelName = data[filename];
                    const titleText = `${baseTitle} - ${modelName}`;
