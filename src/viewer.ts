...
export class Viewer {
	constructor(el, options) {
		...
		// --- 确保父容器有定位上下文，以便 Loading 遮罩层能绝对定位覆盖 ---
	...
	createSpinner() {
		// 1. 注入 CSS 样式
		if (!document.getElementById('viewer.vue-loader-style')) {
			const style = document.createElement('style');
			style.id = 'viewer.vue-loader-style';
			style.innerHTML = `
             .viewer-loader-overlay {
                position: absolute; top: 0; left: 0; width: 100%; height: 100%;
                background: rgba(255, 255, 255, 0.8);
                display: flex; 
                flex-direction: column; /* 修改：改为垂直排列 */
		...
		// 注意：className 里不要带 '.'，否则会导致样式/查询选择器全部失效
		this.spinnerEl.className = 'viewer-loader-overlay';
		// 修改：增加 text 元素
		...
		// 保存文字元素的引用，方便后续更新
		this.progressEl = this.spinnerEl.querySelector('.viewer-loader-text');
	}

	// --- 更新进度文字 ---
	updateProgress(percent) {
		if (this.progressEl) {
			this.progressEl.innerText = `${Math.floor(percent)}%`;
		}
	}
	...
	// --- 隐藏 Loading ---
	hideLoading() {
		if (this.spinnerEl) this.spinnerEl.classList.remove('visible');
	}
	...
	load(modelkey, rootPath, assetMap, onProgress) {
		// --- 开始加载时显示 Loading ---
		this.showLoading();
		if (typeof onProgress === 'function') onProgress(0);
		// 返回 Promise，以便外部调用者知道何时加载结束
		return new Promise((resolve, reject) => {
			// 1. 等待 loadModelInfo 完成，获取最终的 URL
			this.loadModelInfo(modelkey)
				.then((url) => {
					console.log('loadModelInfo returned URL:', url);
					if (!url.includes('/')) {
						url = `${import.meta.env.BASE_URL === '/' ? '.' : import.meta.env.BASE_URL}/models/${url}`;
					}
					console.log('Final model URL:', url);
					// 2. 获取到正确的 url 后，执行原有的加载逻辑
					const baseURL = LoaderUtils.extractUrlBase(url);

					MANAGER.setURLModifier((uri, path) => {
					...
					loader.load(
						...
						(gltf) => {
							// --- 加载成功 ---
							console.log('GLTFLoader loaded successfully:', gltf);
							this.hideLoading();
							...
							console.log('Setting content, scene:', scene);
				...
				.catch((err) => {
					// 如果 loadModelInfo 自身发生了极其严重的错误（通常 catch 块已处理，这里是兜底）
					this.hideLoading();
					reject(err);
				});
		});
	...
	/**
	 * 功能：解析 URL 文件名，请求 models/info.json，并更新 H1 标签
	 */
	loadModelInfo(modelKey) {
		return new Promise((resolve) => {
			let finalUrl = modelKey;
			const filename = modelKey.substring(modelKey.lastIndexOf('/') + 1);
			const baseTitle = webBaseTitle;
			fetch(`${import.meta.env.BASE_URL === '/' ? '.' : import.meta.env.BASE_URL}/models/info.json`)
				.then((res) => {
					if (!res.ok) throw new Error('Info file not found');
					return res.json();
				})
				.then((data) => {
					// 优先匹配文件名，如果找不到，再尝试匹配完整的 URL
					const modelInfo = data[filename] || data[modelKey];

					if (modelInfo) {
						const titleText = `${baseTitle} - ${modelInfo.name}`;
						...
						// 如果 info.json 里配置了新的 url，则替换
						if (modelInfo.url) {
							finalUrl = modelInfo.url;
						}
					} else {
						// 未匹配到，重置为默认标题
						const headerEl = document.querySelector('header h1');
						if (headerEl) headerEl.textContent = baseTitle;
				...
				.catch((err) => {
					console.log('Model info not loaded:', err.message);
					// 即使获取 info 失败，也要 resolve 原始 URL，保证模型能继续加载
					resolve(finalUrl);
				});
		});
	}
	...
	setContent(object, clips) {
		...
		object.updateMatrixWorld(); // donmccurdy/three-gltf-viewer.vue#330
	...
	addGUI() {
		...
		// 清理并重建 GUI 容器，避免 dat.gui 多层 .dg 元素被全局样式误伤导致重叠
		const existingRoot = document.getElementById('viewer-gui-root');
		...
		// 将 GUI 添加到专用容器中（容器负责定位到右下角）
		guiRoot.appendChild(gui.domElement);

		// 默认关闭（需要时用户可手动展开；kiosk 模式同样保持关闭）
		gui.close();
	}
